...
export async function isInindiaKashmir(marker, result) {
  //function to check if the marker is in disputed kashmir
  if (bbox.contains(marker.getLatLng())) {
    const addressData = await fetch(
      `${geocodingAPI}/details?osmtype=${result.osm_type.trim().charAt(0).toUpperCase()}&osmid=${result.osm_id}&addressdetails=1&format=json`,
      headerofNominatim,
    ).then((response) => response.json());
    if (
      leafletPip.pointInLayer(marker.getLatLng(), L.geoJson(kasmir)).length > 0 &&
      addressData.country_code != 'in'
    ) {
      //make no data for pak-kasmir
      return true;
    } else {
      return false;
    }
