...
export const osmIds = [
  //osm ids of kasmir included parts of china and pak
  ...
  const timeoutPromise = new Promise((_, reject) =>
    setTimeout(() => {
      if (!cancelFetch && currentSearch === SearchID) {
        // Check if the last called function is the current one
...
async function geoJSON(type, id) {
  ...
  if (
    leafletPip.pointInLayer(L.latLng(centre), (geoLayer = L.geoJson(result, Options))).length <= 0
  ) {
