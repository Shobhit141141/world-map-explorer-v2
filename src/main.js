...
import {
  closeDistanceFinder,
  initialize_DistanceFinder_EventListeners,
} from './components/DistanceFinder/distance.js';
...
function handleKeyDownOnDocument(event) {
...
function toggleKeyboardShortcuts() {
  ...
  if (keys) {
    if (keys.style.display === 'none' || keys.style.display === '') {
      ...
      closeSound.play();
      document.removeEventListener('keydown', lockTabKeyRefer); // Remove tab key lock
    }
  } else {
...
<ol>
    <li><strong>H</strong>: Toggle keyboard shortcuts</li>
